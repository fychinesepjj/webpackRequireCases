...
* 分别加载了__JS__/utils文件，此时对其进行预加载，__JS__/utils与__COMP__/header/header会被一同抽取到parent.trunk.js中，
* 在预加载同时对公共加载项进行了提取。因此建议，如果在require.ensure回调中再次使用异步加载方法时，应优先考虑使用预加载提取公共文件。
